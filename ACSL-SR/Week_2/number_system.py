...
# Write functions to convert between decimal, binary, hexadecimal, and octal numbers.
# Each function takes in a string representing a number in onenumber system
# and returns a string representing the number in another number system.
# Use lower case letters for the digits above 9, e.g., a = 10, b = 11, etc.


# Convert a decimal number to a binary number
#see the maximum binary digit threshold for dec input
#apply modulus 2 to get remainders and go on
#remember: smallest case is 1 2^^0
def dec_to_bin(dec):
    #print("START DEC:",dec)
...
def bin_to_dec(bin_str):
    dec = 0
    for index_num in range(len(bin_str)):
        #print("index_num:", index_num)
        num = int(bin_str[index_num])
        #print("num:", num)
        #index numbering (0-based) and exponent numbering (0-based) match up
        dec += num * 2** (len(bin_str)-1 - index_num)
        #print("dec:",dec)
    #print("Final dec:",dec)
    return dec
...
def hex_to_dec(hex_str):
    # your code here
    dec = 0
    for index_num in range(len(hex_str)):
        #print("index_num:", index_num)
        if hex_str[index_num].isnumeric():
            num = int(hex_str[index_num])
        else:
            char = hex_str[index_num].upper()
            if char == "A":
                num = 10
            elif char == "B":
                num = 11
                ...
                num = 15
        dec += num * 16 ** (len(hex_str)-1 - index_num)
...
def oct_to_dec(oct_str):
    dec = 0
    for index_num in range(len(oct_str)):
        num = int(oct_str[index_num])
        dec += num * 8** (len(oct_str)-1 - index_num)
    return dec
...
def oct_to_hex(oct_str):
    # your code here
    dec_val = oct_to_dec(oct_str)
    #print("Dec:",dec_val)
    return dec_to_hex(dec_val)
#print("Finald:",oct_to_hex('12'))
#should be 10
#print("Final:",oct_to_hex("20"))
#should be 10
